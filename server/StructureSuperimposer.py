...
if __name__ == '__main__':
  ...
  for test_filename in os.listdir(TEST_CASES_DIR):
    with open(
        os.path.join(TEST_CASES_DIR, test_filename), 'r', encoding='utf-8'
    ) as test_file:
      test_case = test_file.read()

    DIFF_PREFIX = 'diff --git'
    [_prev_source, _diff] = test_case.split(DIFF_PREFIX)
    _diff = DIFF_PREFIX + _diff

    structure_superimposer = StructureSuperImposer.from_diff(
      _prev_source, _diff,
    )
    print(f'{test_filename}:')
    print(structure_superimposer.to_bracketed_notation('\n</s>'), '\n')
